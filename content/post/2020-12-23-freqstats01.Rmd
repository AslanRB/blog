...
```{r probdists unif}
p.d = data.frame(x = 0:10, y = dunif(0:10,0,10)) %>%
  ggplot() + theme_bw() +
  coord_cartesian(xlim=c(-1,11), y=c(0,0.11))+
  scale_x_continuous(breaks=0:10, name=expression(D))+
  scale_y_continuous(breaks=c(.05,.1), name=expression(Probability))+
  geom_segment(aes(x=x, xend=x, y=-1, yend=y), size=1)+
  ggtitle("Discrete")+
...
p.c = data.frame(x = 0:10, y = dunif(0:10,0,10)) %>%
  ...
  scale_y_continuous(breaks=c(.05,.1), name=expression(Probability~Density))+
  geom_path(aes(x=x, y=y), size=1)+
  geom_segment(aes(x=0, xend=0, y=-1, yend=0.1), size=0.5)+
  geom_segment(aes(x=10, xend=10, y=-1, yend=0.1), size=0.5)+
  ggtitle("Continuous")+
  theme(axis.title=element_text(size=9),
        plot.title=element_text(size=9),
        panel.grid=element_blank())
