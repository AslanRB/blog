...
# Packages
library(knitr); library(tidyverse); library(patchwork); library(ggforce)
...
$\mathbb{R}$ Did this work???

```{r sample space}
p.d = ggplot() + theme_void () + coord_fixed(ratio=0.5) +
  geom_rect(aes(xmin=0, xmax=1, ymin=0, ymax=1), col="black", fill=NA, size=1) +
  geom_point(aes(x=0.5:9.5/10, y=0.5), shape=21, col="gray50", fill="gray50", size=2) +
  geom_segment(aes(x=1:9/10, xend=1:9/10, y=0, yend=1))+
  ggtitle("  Discrete r.v.")+
  annotate("text", label="1", hjust=0.5, vjust=1, x=0.05, y=0.4, size=4)+
  annotate("text", label="2", hjust=0.5, vjust=1, x=0.15, y=0.4, size=4)+
  annotate("text", label="3", hjust=0.5, vjust=1, x=0.25, y=0.4, size=4)+
  annotate("text", label="4", hjust=0.5, vjust=1, x=0.35, y=0.4, size=4)+
  annotate("text", label="5", hjust=0.5, vjust=1, x=0.45, y=0.4, size=4)+
  annotate("text", label="6", hjust=0.5, vjust=1, x=0.55, y=0.4, size=4)+
  annotate("text", label="7", hjust=0.5, vjust=1, x=0.65, y=0.4, size=4)+
  annotate("text", label="8", hjust=0.5, vjust=1, x=0.75, y=0.4, size=4)+
  annotate("text", label="9", hjust=0.5, vjust=1, x=0.85, y=0.4, size=4)+
  annotate("text", label="10", hjust=0.5, vjust=1, x=0.95, y=0.4, size=4)+
  theme(text=element_text(size=9))

p.c = ggplot() + theme_void () + coord_fixed(ratio=0.5) +
  geom_segment(aes(x=0, xend=1, y=0.5, yend=0.5), col="gray50", size=2) +
  geom_rect(aes(xmin=0, xmax=1, ymin=0, ymax=1), col="black", fill=NA, size=1)+
  ggtitle("  Continuous r.v.")+
  geom_segment(aes(x=0, xend=0.03, y=0.5, yend=0.4))+
  geom_segment(aes(x=1, xend=0.97, y=0.5, yend=0.4))+
  annotate("text", label="0", hjust=0, vjust=1, x=0.03, y=0.4, size=4)+
  annotate("text", label="10", hjust=1, vjust=1, x=0.97, y=0.4, size=4)+
  #geom_segment(aes(x=0.1, xend=0.87, y=0.35, yend=0.35))+
  theme(text=element_text(size=9))
...
```{r probdists unif}
p.d = data.frame(x = 1:10, y = dunif(1:10,0,10)) %>%
  ggplot() + theme_bw() +
  coord_cartesian(xlim=c(0,11), y=c(0,0.11))+
  scale_x_continuous(breaks=1:10, name=expression(D))+
  scale_y_continuous(breaks=c(0,.1), name=expression(Probability))+
  geom_segment(aes(x=x, xend=x, y=0, yend=y), size=1)+
  geom_segment(aes(x=-10, xend=20, y=0, yend=0), linetype="dotted")+
  ggtitle("Discrete")+
  theme(axis.title=element_text(size=9),
        plot.title=element_text(size=9),
        panel.grid=element_blank())

p.c = data.frame(x = 0:10, y = dunif(0:10,0,10)) %>%
  ggplot() + theme_bw() +
  coord_cartesian(xlim=c(-1,11), y=c(0,0.11))+
  scale_x_continuous(breaks=0:10, name=expression(C))+
  scale_y_continuous(breaks=c(0,.1), name=expression(Probability~Density))+
  geom_path(aes(x=x, y=y), size=1)+
  geom_path(aes(x=-10:0, y=0), size=1)+
  geom_path(aes(x=10:20, y=0), size=1)+
  geom_segment(aes(x=0, xend=0, y=0, yend=0.1), size=0.5)+
  geom_segment(aes(x=10, xend=10, y=0, yend=0.1), size=0.5)+
  geom_segment(aes(x=-10, xend=20, y=0, yend=0), linetype="dotted")+
  ggtitle("Continuous")+
  theme(axis.title=element_text(size=9),
        plot.title=element_text(size=9),
...
set.seed(2021)
n = 650; coins = 5
flips = replicate(rbinom(n,1,.5), n=coins)
chead = flips
for(i in 1:coins){
  for(j in 1:n){
    chead[j,i] = mean(flips[1:j,i])
  }
}

data.frame(chead = as.vector(chead),
           coin  = as.factor(rep(1:coins, each=n)),
           flip  = rep(1:n, coins)) %>%
  
  ggplot(aes(x=flip, y=chead, group=coin)) + theme_bw() +
  coord_cartesian(xlim=c(1,n-50), ylim=c(0,1))+
  scale_x_continuous(breaks=c(1,seq(100,n-50,100)), name="Number of Tosses")+
  scale_y_continuous(breaks=seq(0,1,0.5), name="Running Proportion Heads")+
  geom_path(color="gray50") +
  geom_hline(yintercept=0.5, linetype="longdash", color="darkred") +
  theme(text=element_text(size=9),
        panel.grid=element_blank())
...
```{r dice, eval=F}
set.seed(2021)
roll <- function(){d1 = sample(1:6,1); d2 = sample(1:6,1); return((d1+d2) %% 3 == 0)} #T if multiple of 3

reps=10000; outcomes=replicate(reps, roll()); cprob=vector(length=reps)
for(i in 1:reps) cprob[i] = mean(outcomes[1:i])

data.frame(x=1:800, y=cprob[1:800]) %>%
  ggplot(aes(x=x, y=y))+
  geom_hline(yintercept=1/3, color="gray40")+
  geom_line(size=0.5)+
  theme_classic()+
  ggtitle("Throwing 2 Dice")+
  coord_fixed(xlim=c(1,650), ylim=c(0,1), ratio=200)+
  scale_x_continuous(name="Throws", breaks=c(1,100,200,300,400,500,600))+
  scale_y_continuous(name="Proportion Multiples of 3", breaks=seq(0,1,0.2))+
  theme(axis.title=element_text(size=9),
        plot.title=element_text(size=9),
...
data.frame(x=(reps-200):reps, y=cprob[(reps-200):reps]) %>%
  ggplot(aes(x=x, y=y))+
  geom_hline(yintercept=1/3, color="gray40")+
  geom_line(size=0.5)+
  theme_classic()+
  ggtitle("Throwing 2 Dice")+
  coord_fixed(xlim=c(reps-150,reps+10), ylim=c(0,1), ratio=200)+
  scale_x_continuous(name="Throws", breaks=c(reps-100,reps))+
  scale_y_continuous(name="Proportion Multiples of 3")+
  theme(axis.title=element_text(size=9),
...
```{r coin flips, fig.height=5}
set.seed(2020)

# Plot 1
flips = 6; outcome = rbinom(flips,1,0.5);

p.11 = data.frame(index=1:flips, outcome) %>%
  ggplot(aes(x=outcome, y=index))+
  geom_vline(xintercept=0:1, color="gray90")+
  geom_point(size=2, shape=21, color="black", fill="black")+
  scale_y_reverse(name="Repetition #", limits=c(flips,0), breaks=1:flips)+
  scale_x_continuous(name="k / N", limits=c(-0.1,1.1), breaks=c(0,0.5,1))+
  theme_bw()+
  ggtitle("N=1")+
  theme(axis.title=element_text(size=9),
        plot.title=element_text(size=9),
...
p.12 = data.frame(x=(0:1)/1, y=dbinom(0:1,1,0.5)) %>%
  ...
  theme_bw()+
  ggtitle(expression(paste(infinity," Repetitions")))+
  theme(axis.title=element_text(size=9),
        plot.title=element_text(size=9),
        #axis.ticks.x=element_blank(),
        #axis.text.x=element_blank(),
...
# Plot 2
flips = 4; runs = 6; outcome = replicate(runs, rbinom(flips,1,0.5)); cprop = index = outcome;
for(f in 1:flips) for(r in 1:runs) {cprop[f,r] = mean(outcome[1:f,r]); index[f,r] = r + (f-flips)*0.8/flips}

p.21 = data.frame(index=as.vector(index), outcome=as.vector(cprop), run=as.factor(rep(1:runs,each=flips))) %>%
  ggplot(aes(x=outcome, y=index, group=run))+
  geom_vline(xintercept=0:flips/flips, color="gray90")+
  geom_path(size=0.5, color="black")+
  geom_point(size=1, shape=21, color="black", fill="white")+
  geom_point(size=2, shape=21, color="black", fill="black", aes(y=ifelse(index%%1==0,index,NA), x=outcome))+
  scale_y_reverse(name="Repetition #", limits=c(runs,0), breaks=1:runs)+
  scale_x_continuous(name="k / N", limits=c(-0.1,1.1), breaks=c(0,0.5,1))+
  theme_bw()+
  ggtitle("N=4")+
  theme(axis.title=element_text(size=9),
...
p.22 = data.frame(x=(0:flips)/flips, y=dbinom(0:flips,flips,0.5)) %>%
  ggplot(aes(x=x, y=y, group=x))+
  geom_vline(xintercept=0:flips/flips, color="gray90")+
  geom_segment(aes(x=x, xend=x, y=0, yend=y), size=1)+
  theme_bw()+
...
p.31 = data.frame(index=as.vector(index), outcome=as.vector(cprop), run=as.factor(rep(1:runs,each=flips))) %>%
  ggplot(aes(x=outcome, y=index, group=run))+
  geom_vline(xintercept=0:flips/flips, color="gray90")+
  geom_path(size=0.5, color="black")+
  geom_point(size=1, shape=21, color="black", fill="white")+
...
p.32 = data.frame(x=(0:flips)/flips, y=dbinom(0:flips,flips,0.5)) %>%
  ggplot(aes(x=x, y=y, group=x))+
  geom_vline(xintercept=0:flips/flips, color="gray90")+
  geom_segment(aes(x=x, xend=x, y=0, yend=y), size=1)+
  theme_bw()+
...
((p.11 / p.12) | (p.21 / p.22) | (p.31 / p.32)) +
  plot_annotation(subtitle = "Process: N coin flips -- Outcome: k Heads / N") &
  theme(text = element_text(size=9))
