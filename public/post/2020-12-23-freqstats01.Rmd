...
ggplot(mapping=aes(x=x,y=y))+
  ...
  annotate("text", hjust=0, vjust=0.5, x=-31, y=0.07, size=3, label=expression(paste("Normal (",mu,", ",sigma,")")))+
  ...
  annotate("text", hjust=0, vjust=0.5, x=-22, y=0.29, size=3, label=expression(paste("Normal (",mu,", ",sigma/sqrt(N),")")))
...
p.1a = ggplot(data=samdis1)+
  ...
  annotate("rect", size=1, col=col.red, fill=col.red, alpha=filler, xmin=xlims[2]-1, xmax=xlims[2], ymin=0.15-1/ratio, ymax=0.15)+
  annotate("text", size=3, hjust=1, vjust=0.5, x=xlims[2]-1, y=0.15-0.5/ratio, label=expression(paste(1-beta(4)," = ")))
...
p.2a = ggplot(data=samdis2)+
  ...
  annotate("text", size=3, hjust=1, vjust=0.5, x=xlims[2]-1, y=0.27-0.5/ratio, label=expression(paste(1-beta(4)," = ")))
...
ggplot(dat, aes(x=x, y=n))+
  ...
  # Design
  scale_x_continuous(limits=xlims, breaks=xbrks, name=expression(paste(X-mu[0])))+
...
ggplot(dat, aes(x=xd, y=n))+
  ...
  geom_vline(xintercept=mu0, color="white", linetype="dashed")+
  # Design
  scale_x_continuous(limits=xlims, breaks=xbrks, name=expression(paste("(",X-mu[0],")/",sigma)))+
  scale_y_continuous(limits=ylims, breaks=ybrks, name="")+
  theme_bw()+
  theme(axis.title=element_text(size=9),
        legend.title=element_text(size=8),
